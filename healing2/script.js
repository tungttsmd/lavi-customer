$(document).ready(function () {
  ...
  $(
    '.grid > div.box-animate, .bg-white.rounded-2xl, .testimonial-card'
  ...
  $('#newsletter-form').submit(function (e) {
    ...
    if (validateEmail(email)) {
      // Simulate API call
      messageDiv.html(
        '<div class="message success">Thank you for subscribing to the newsletter!</div>'
      )
      $(this)[0].reset()
    } else {
      messageDiv.html(
  ...
  $(
    '.book-session-btn, button:contains("Book Session"), button:contains("Book your Session")'
  ).click(function (e) {
    e.preventDefault()
    showBookingModal()
  })
  ...
  window.closeBookingModal = function () {
    $('#booking-modal .bg-white')
      .addClass('scale-95 opacity-0 hidden')
  ...
  $('button').on('click', function (e) {
    ...
    const ripple = $(
      `<span class="ripple" style="width: ${size}px; height: ${size}px; left: ${x}px; top: ${y}px;"></span>`
    )
  ...
  console.log(
    '%c🌟 Welcome to Healing Vibes! 🌟',
    'color: #bf9899; font-size: 18px; font-weight: bold;'
  )
  console.log(
    '%cBuilt with love using jQuery and Tailwind CSS',
...
window.addEventListener('scroll', () => {
  const flower = document.getElementById('scroll-flower')
...
window.addEventListener('scroll', () => {
  ...
  const offset = scrollY *0.1 // Điều chỉnh độ trôi nhẹ (10%)
